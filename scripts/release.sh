...
XCODE_PROJECT="${PROJECT_ROOT}/rawctl.xcodeproj"
BUILD_DIR="${PROJECT_ROOT}/build"
ARCHIVE_PATH="${BUILD_DIR}/${ARTIFACT_PREFIX}.xcarchive"
...
R2_BUCKET="${R2_BUCKET:-rawctl-releases}"
RELEASE_TARGET="${RELEASE_TARGET:-$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)}"
...
APP_BUNDLE_PATH="${EXPORT_PATH}/${APP_NAME}.app"
...
if [ ! -f "${XCODE_PROJECT}/project.pbxproj" ]; then
    print_error "Must run from the repository root"
    exit 1
fi
...
SIGNATURE=""
if [ -f "${BIN_DIR}/sign_update" ]; then
    SIGNATURE=$("${BIN_DIR}/sign_update" "${DMG_PATH}" 2>&1 | perl -ne 'print "$1\n" if /sparkle:edSignature="([^"]+)"/' || echo "")
...
BUILD_NUMBER=$(/usr/libexec/PlistBuddy -c "Print :ApplicationProperties:CFBundleVersion" "${ARCHIVE_PATH}/Info.plist" 2>/dev/null || echo "1")
PUB_DATE=$(date -R)
...
if [ "$SKIP_UPLOAD" = true ]; then
    ...
    if ! command -v wrangler &> /dev/null; then
        ...
        wrangler r2 object put "${R2_BUCKET}/${ARTIFACT_PREFIX}-latest.dmg" \
            --file="${RELEASE_DIR}/${DMG_NAME}" \
            --content-type="application/octet-stream" \
            --remote
        ...
        print_info "Latest:  ${APPCAST_URL}/${ARTIFACT_PREFIX}-latest.dmg"
    fi
fi
...
### Downloads
- **DMG**: [${DMG_NAME}](${APPCAST_URL}/${DMG_NAME})
- **Latest**: [${ARTIFACT_PREFIX}-latest.dmg](${APPCAST_URL}/${ARTIFACT_PREFIX}-latest.dmg)
...
echo ""
echo "Next steps:"
echo "  1. Verify CHANGELOG.md / README.md / landing copy for this release"
echo "  2. Build and deploy landing: cd landing && npm run build"
echo "  3. Upload release artifacts if this was a dry run"
echo "  4. Test update in app: Latent → Check for Updates"
echo ""
echo "URLs:"
echo "  Appcast:  ${APPCAST_URL}/appcast.xml"
echo "  DMG:      ${APPCAST_URL}/${DMG_NAME}"
echo "  GitHub:   https://github.com/888wing/latent/releases/tag/v${VERSION}"
echo ""
